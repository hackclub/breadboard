...
const approvalGuidance = [
  `What was built and which platforms it was tested on. "This is a project that does X, Y. I tested Z and it works and feels polished."`,
  "Notable deliverables (e.g. binaries, demo video, documentation quality) and why they support the approved scope.",
  `What evidence you examined as the reviewer. "They included a video that showed X working in Y context."`,
  `Commit count as evidence that the scope is consistent with the approved hours. "They had X commits over a Y period, which looks appropriate."`,
  `Any hour adjustments: the original claimed time, what was deducted and why, and the final approved amount. "Adjusting hours by X due to Y."`,
  "Check the README for context. Beginners take longer. Check for viral posts or high star counts.",
  `If the project is high quality explain why. "This project is cool/high quality because it has/does X, Y, and Z."`,
];
...
export function ReviewWorkspace({
  ...
  const approvedCredits = Math.max(0, Math.floor(approvedHours || 0)) * 30;
        ...
        <div className="flex items-start justify-between gap-4 border-b border-black/10 p-5">
          ...
          <div className="relative hidden h-28 w-52 shrink-0 overflow-hidden rounded-[12px] border border-black bg-white lg:block">
            {screenshot ? (
              ...
              <div className="grid h-full place-items-center text-xs font-black text-black/30">
                No image
              </div>
            )}
          </div>
        ...
        <div className="p-5">
          {tab === "verdict" ? (
            <div className="space-y-5">
              ...
              <div className="rounded-[12px] border border-[#BD0F32]/20 bg-[#fff5f7] p-4">
                <div className="flex items-center gap-2">
                  <HiLightBulb className="size-5 text-[#BD0F32]" />
                  <p className="text-sm font-black text-[#BD0F32]">
                    What your approval message should cover
                  </p>
                </div>
                <ul className="mt-3 space-y-2 text-sm text-black/65">
                  {approvalGuidance.map((item) => (
                    <li key={item} className="flex gap-2">
                      <span className="mt-0.5 shrink-0 text-[#BD0F32]">›</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="grid gap-4">
                  <label className="grid gap-1.5">
                    ...
                    <span className="text-sm font-black text-[#BD0F32]">
                      Awards {approvedCredits} coins ({approvedHours || 0}h ×
                      30)
                    </span>
                  </label>
                  <label className="grid gap-1.5">
                  ...
                  >
                    Submit approval · {approvedCredits} coins
                  </button>
                </div>
              ) : (
      ...
      <aside className="space-y-4">
        <section className="rounded-[16px] border border-black bg-black p-4 shadow-[4px_4px_0_#BD0F32]">
          ...
          <div className="mt-3 space-y-2 text-sm text-white/65">
            ...
            <div className="flex justify-between">
              <span>Coins possible</span>
              <span className="font-black text-white">
                {initial.hoursSpent * 30}
              </span>
            </div>
            ...
            {initial.creditedAmount > 0 ? (
              <div className="flex justify-between">
                <span>Credited</span>
                <span className="font-black text-[#BD0F32]">
                  {initial.creditedAmount} coins
                </span>
              </div>
            ) : null}
          </div>
        ...
        <section className="rounded-[16px] border border-black bg-white p-4 shadow-[4px_4px_0_#000]">
          ...
          <p className="mt-3 text-3xl font-black text-black">
            {approvedCredits} coins
          </p>
          <p className="mt-1 text-sm text-black/45">
            {approvedHours || 0}h × 30
          </p>
        </section>

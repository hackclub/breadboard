...
export function DemoReviewWorkspace({
  ...
  function run(action: () => Promise<void>) {
    ...
    startTransition(async () => {
      try {
        await action();
        setSubmitted(true);
        // Advance to the next still-pending card in the demo lane, resolved now
        // rather than at page load: on a busy queue the target picked when this
        // page opened may already be decided, and we must never drop the
  ...
  // next still-pending demo card, carrying the skip set in the URL so skipped
  // cards don't resurface this session. Falls back to the gallery.
  function skipReview() {
    setErrorMsg(null);
    const carry = [initial.id, ...skipIds];
    startTransition(async () => {
      const href = await nextPendingReviewHref("demo", carry, carry).catch(

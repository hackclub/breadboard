...
import {
  ...
  BLANK_CHIP,
  type ChipExample,
} from "@/components/velxio/components/customChips/chipExamples";
import {
  compileChip,
  type ChipCompileResult,
} from "@/services/velxio/services/chipCompileService";
...
export const CustomChipDialog = ({
  initial,
  onClose,
  onSave,
}: CustomChipDialogProps) => {
  const { t } = useTranslation();
  // If the chip already has source code, skip the examples tab on open.
  const [tab, setTab] = useState<Tab>(
    initial.sourceC.trim() ? "editor" : "examples",
  );
  ...
  const [chipJson, setChipJson] = useState(
    initial.chipJson || BLANK_CHIP.chipJson,
  );
  ...
  useEffect(() => {
    setChipName((prev) => {
      if (prev && prev.trim() && !BLANK_CHIP_NAMES.has(prev.trim()))
        return prev;
      try {
        const obj = JSON.parse(chipJson);
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={dialogStyle} onClick={(e) => e.stopPropagation()}>
        <div style={headerStyle}>
          <strong style={{ flex: 1 }}>
            {t("editor.customChip.title", { chipName })}
          </strong>
          ...
          <div style={resultPanelStyle}>
            {compiling && (
              <span style={{ color: "#ffa500" }}>
                {t("editor.customChip.compiling")}
              </span>
            )}
            {!compiling && result && result.success && (
              <span style={{ color: "#22c55e" }}>
                {t("editor.customChip.compiledOk", {
                  kb: (result.byte_size / 1024).toFixed(1),
                })}
              </span>
            )}
            {!compiling && result && !result.success && (
        ...
        <div style={footerStyle}>
          ...
          <button
            style={canSave ? saveBtn : saveBtnDisabled}
            disabled={!canSave}
            onClick={doSave}
          >
            {canSave
              ? t("editor.customChip.savePlace")
              : t("editor.customChip.compileFirst")}
          </button>
        </div>
      </div>
    </div>

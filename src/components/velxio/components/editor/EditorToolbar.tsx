...
} from "@/lib/velxio/simulation/verify/circuitVerifier";
import { buildInputFromStore } from "@/lib/velxio/simulation/spice/storeAdapter";
import { BOARD_PIN_GROUPS } from "@/lib/velxio/simulation/spice/boardPinGroups";
import { CircuitVerificationModal } from "@/components/velxio/components/simulator/CircuitVerificationModal";
import type { PinSourceState } from "@/lib/velxio/simulation/spice/types";
import type { BoardKind, LanguageMode } from "@/lib/velxio/types/board";
import {
  BOARD_KIND_FQBN,
...
import { compileCode } from "@/services/velxio/services/compilation";
import {
  compileRom,
  isChipProgramFile,
  formatForFile,
...
import { compileChip } from "@/services/velxio/services/chipCompileService";
...
import { reportRunEvent } from "@/services/velxio/services/metricsService";
...
import "@/components/velxio/components/editor/EditorToolbar.css";
...
interface EditorToolbarProps {
  ...
  setCompileLogs: (
    logs: CompilationLog[] | ((prev: CompilationLog[]) => CompilationLog[]),
  ) => void;
  /**
...
const _BOARD_PILL_COLOR: Record<BoardKind, string> = {
  "arduino-uno": "#4fc3f7",
  "arduino-nano": "#4fc3f7",
  "arduino-mega": "#4fc3f7",
...
export const EditorToolbar = ({
  ...
  const prepareCustomChips = useCallback(
    async (
      ...
      for (const chip of chips) {
        ...
        const live = useSimulatorStore
          .getState()
          .components.find((c) => c.id === chip.id);
        const props = { ...(live?.properties ?? chip.properties) } as Record<
          string,
          unknown
        >;
        ...
        const chipTarget: CompileTarget = {
          id: chip.id,
          label: chipLabel,
          kind: "chip",
        };
        ...
        if (!String(props.wasmBase64 ?? "") && sourceC) {
          ...
          try {
            ...
            if (r.success && r.wasm_base64) {
              ...
              clog(
                "success",
                `Chip "${chipLabel}" compiled (${r.byte_size} B WASM).`,
              );
            } else {
              clog(
                "error",
          ...
          if (!file) {
            ...
            try {
              ...
              if (rr.success && rr.rom_base64) {
                ...
                clog(
                  "success",
                  `ROM ready: ${rr.byte_size} B injected into "${chipLabel}".`,
                );
              } else {
                clog(
                  "error",
  ...
  const handleCompile = async () => {
    ...
    const customChips = componentsForCompile.filter(
      (c) => c.metadataId === "custom-chip",
    );
    ...
    if (activeBoard?.languageMode === "micropython" && activeBoardId) {
      ...
      try {
        const groupFiles = useEditorStore
          .getState()
          .getGroupFiles(activeBoard.activeFileGroupId);
        const pyFiles = groupFiles.map((f) => ({
          name: f.name,
          content: f.content,
        }));
        ...
        const errMsg =
          err instanceof Error ? err.message : "Failed to load MicroPython";
        blog("error", errMsg);
    ...
    try {
      ...
      const sketchFiles = (groupFiles.length > 0 ? groupFiles : files)
        ...
        .filter(
          (f) => !chipProgramFiles.has(f.name) && !isChipProgramFile(f.name),
        )
        .map((f) => ({
          name: f.name,
          content: f.content,
      ...
      if (result.success) {
        ...
        const looksLikeMissingLib =
          /No such file or directory|fatal error:.*\.h|library not found/i.test(
            errText,
          );
  ...
  const runVerification =
    useCallback(async (): Promise<VerificationResult | null> => {
      try {
        ...
        const snap = {
          ...
          boards: sim.boards.map((b) => {
            ...
            const group =
              BOARD_PIN_GROUPS[b.boardKind] ?? BOARD_PIN_GROUPS.default;
            const wiredPinNames = new Set<string>();
            for (const w of sim.wires) {
              if (w.start.componentId === b.id)
                wiredPinNames.add(w.start.pinName);
  ...
  const handleRun = async (skipVerify = false) => {
    ...
    if (activeBoardId) {
      ...
      if (isQemuBoard) {
        ...
        reportRun(board?.boardKind);
        console.log(
          "[handleRun] → startBoard (already compiled)",
          activeBoardId,
        );
        startBoard(activeBoardId);
        setMessage(null);
  ...
  const handleStop = () => {
    ...
    const runningBoards = useSimulatorStore
      .getState()
      .boards.filter((b) => b.running);
   ...
   */
  const compileAllBoards = async (): Promise<{
    ok: number;
    failed: number;
  }> => {
    ...
    const allCustomChips = useSimulatorStore
      .getState()
      .components.filter((c) => c.metadataId === "custom-chip");
    if (boardsList.length === 0 && allCustomChips.length === 0)
    ...
    const targetSummary = [
      boardsList.length
        ? `${boardsList.length} board${boardsList.length === 1 ? "" : "s"}`
        : "",
  ...
  const handleRunAll = async () => {
    ...
    if (chips.length > 0) {
      useSimulatorStore.getState().restartParts();
      const anyBoardRunning = useSimulatorStore
        .getState()
  ...
  return (
    <>
        ...
        <div className="editor-toolbar" ref={toolbarRef}>
              ...
              <select
                ...
                onChange={(e) => {
                  if (activeBoardId)
                    setBoardLanguageMode(
                      activeBoardId,
                      e.target.value as LanguageMode,
                    );
                }}
          ...
          <div className="toolbar-group">
            ...
            <button
              ...
              title={
                isBoardless
                  ? "Freeze digital simulation"
                  : t("editor.toolbar.stop")
              }
            >
              <svg
                width="18"
          ...
          <div className="toolbar-group toolbar-group-right">
            ...
            >
              ...
              <span className="tb-libraries-label">
      ...
      <LibraryManagerModal
        isOpen={libManagerOpen}
        onClose={() => setLibManagerOpen(false)}
      />

...
export const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const programmableChips = components.filter(
    (c) =>
      ...
      isProgrammableChip(c.properties as Record<string, unknown>),
  );
    ...
    for (const chip of programmableChips) {
      ...
      if (existing) {
        ...
        updateComponent(chip.id, {
          properties: {
            ...props,
            programFile: DEFAULT_CHIP_PROGRAM_FILE,
    ...
    <div className="file-explorer">
      ...
      <div className="file-explorer-list">
            ...
            <div key={board.id} className="fe-board-section">
              ...
              >
                <button
                  ...
                  title={
                    isOpen
                      ? t("editor.fileExplorer.collapse")
                      : t("editor.fileExplorer.expand")
                  }
                >
                ...
                {renamingSection?.id === board.id &&
                renamingSection.kind === "board" ? (
                  <input
              ...
              {isOpen && (
                <div className="fe-board-files">
                  {groupFiles.map((file) => {
                    const isActiveFile =
                      isActiveBoard && file.id === activeFileId;
                    return (
                      <div
                        key={file.id}
        ...
        {programmableChips.map((chip) => {
          ...
          const chipName =
            String(
              (chip.properties as Record<string, unknown>)?.chipName ?? "",
            ).trim() || "Custom Chip";
            ...
            <div key={chip.id} className="fe-board-section">
              ...
              {isOpen && (
                <div className="fe-board-files">
                  {groupFiles.map((file) => {
                    const isActiveFile =
                      isActiveGroup && file.id === activeFileId;
                    return (
                      <div
                        key={file.id}
                        className={`file-explorer-item fe-file-item${isActiveFile ? " file-explorer-item-active" : ""}`}
      ...
      {contextMenu && (
        ...
        >
          <button
            onClick={() =>
              startRename(contextMenu.fileId, contextMenu.boardGroupId)
            }
          >
          ...
          <button
            ...
            onClick={() =>
              handleDelete(contextMenu.fileId, contextMenu.boardGroupId)
            }

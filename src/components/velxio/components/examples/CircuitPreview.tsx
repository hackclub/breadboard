...
import type { ExampleProject, ExampleBoard } from "@/lib/velxio/data/examples";
import { INLINE_SVGS } from "@/components/velxio/components/examples/InlineComponentSVGs";
...
const COMP_DEFS: Record<string, CompDef> = {
  ...
  "wokwi-ntc-temperature-sensor": {
    svg: "ntc-temperature-sensor.svg",
    w: 136,
    h: 72,
  },
  "wokwi-photoresistor-sensor": {
    svg: "photoresistor-sensor.svg",
    w: 174,
    h: 62,
  },
...
const Attiny85InlinePreview: React.FC<{ w: number; h: number }> = ({
  w,
  h,
}) => (
  <svg
...
function getCompDef(type: string, props: Record<string, any>): CompDef {
  if (type === "wokwi-led") {
    const colorSvg =
      LED_COLOR_SVG[(props.color as string)?.toLowerCase()] ?? "wokwi-led.svg";
    return { ...COMP_DEFS["wokwi-led"], svg: colorSvg };
  }
  if (COMP_DEFS[type]) return COMP_DEFS[type];
  const inline = INLINE_SVGS[type];
  if (inline)
    return { svg: "", inline: inline.component, w: inline.w, h: inline.h };
...
export const CircuitPreview: React.FC<CircuitPreviewProps> = ({
  ...
  if (example.boards && example.boards.length > 0) {
    ...
    const boardInComponents = example.components.some((c) =>
      isBoardType(c.type),
    );
    ...
    if (boardDef && !boardInComponents && !isBoardless) {
      ...
      const avgCompY =
        example.components.length > 0
          ? example.components.reduce((s, c) => s + c.y, 0) /
  ...
  const boardKindForCentre =
    !example.boards && example.boardType ? example.boardType : null;

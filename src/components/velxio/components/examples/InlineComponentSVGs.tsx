...
const Diode1N4007: React.FC<InlineSVGProps> = ({ w, h }) =>
  diodeGlyph("1N4007", w, h);
const Diode1N5817: React.FC<InlineSVGProps> = ({ w, h }) =>
  diodeGlyph("1N5817", w, h);
const DiodeZener: React.FC<InlineSVGProps> = ({ w, h }) =>
  diodeGlyph("ZD", w, h);
...
const Opto4N25: React.FC<InlineSVGProps> = ({ w, h }) =>
  optoGlyph("4N25", w, h);
...
const MotorDriverL293D: React.FC<InlineSVGProps> = ({ w, h }) =>
  dipGlyph("L293D", w, h);
...
const OpampLM358: React.FC<InlineSVGProps> = ({ w, h }) =>
  opampGlyph("LM358", w, h);
...
function gateShape(
  ...
  const negated =
    shape === "nand" || shape === "nor" || shape === "xnor" || shape === "not";
  const exclusive = shape === "xor" || shape === "xnor";
  const isOr =
    shape === "or" || shape === "nor" || shape === "xor" || shape === "xnor";
  const isNot = shape === "not";
  const body = isNot ? (
    <polygon
      points="12,10 12,38 46,24"
      fill="#f8f4ee"
      stroke="#2a2a2a"
      strokeWidth="1.5"
    />
  ...
  return (
    ...
    >
      <line x1="0" y1="16" x2="10" y2="16" stroke="#555" strokeWidth="2" />
      {!isNot && (
        <line x1="0" y1="32" x2="10" y2="32" stroke="#555" strokeWidth="2" />
      )}
      {body}
      {exclusive && (
        <path
          d="M 4 8 Q 14 24 4 40"
          fill="none"
          stroke="#2a2a2a"
          strokeWidth="1.5"
        />
...
const GateNand: React.FC<InlineSVGProps> = ({ w, h }) =>
  gateShape("nand", w, h);
...
const GateXnor: React.FC<InlineSVGProps> = ({ w, h }) =>
  gateShape("xnor", w, h);
...
const Reg7805: React.FC<InlineSVGProps> = ({ w, h }) =>
  reg3pinGlyph("7805", w, h);
const RegLM317: React.FC<InlineSVGProps> = ({ w, h }) =>
  reg3pinGlyph("LM317", w, h);

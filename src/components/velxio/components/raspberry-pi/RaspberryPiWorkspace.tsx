...
import { VirtualFileSystem } from "@/components/velxio/components/raspberry-pi/VirtualFileSystem";
import { useVfsStore } from "@/services/velxio/store/useVfsStore";
import {
  getBoardBridge,
  useSimulatorStore,
} from "@/services/velxio/store/useSimulatorStore";
import { attachSlavesFromCanvas } from "@/lib/velxio/simulation/piSlaveScanner";

// Lazy-load PiTerminal so @xterm/xterm is only bundled when needed
const PiTerminal = lazy(() =>
  import("@/components/velxio/components/raspberry-pi/PiTerminal").then(
    (m) => ({ default: m.PiTerminal }),
  ),
);
...
export const RaspberryPiWorkspace: React.FC<RaspberryPiWorkspaceProps> = ({
  ...
  const board = useSimulatorStore((s) =>
  ...
  const handleConnect = () => {
    ...
    if (bridge && !bridge.connected) {
      bridge.connect();
      setTimeout(
        () => setBridgeConnected(getBoardBridge(boardId)?.connected ?? false),
        500,
      );
    }
  };
            ...
            {!board?.running ? (
              ...
              <>
                <button
                  style={{
                    ...styles.toolbarBtn,
                    color: bridgeConnected ? "#888" : "#4fc3f7",
                  }}
        ...
        <div style={styles.pane}>
          ...
          {!board?.running && (
            <div style={styles.offlineOverlay}>
              <div style={styles.offlineBox}>
                ...
                <div style={styles.offlineTitle}>
                  {t("editor.pi.offlineTitle")}
                </div>
          ...
          {activePane === "terminal" ? (
            ...
            <Editor
              ...
              height="100%"
              language={
                activeFileNode.name.endsWith(".py") ? "python" : "shell"
              }
              theme="vs-dark"

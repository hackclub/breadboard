...
import {
  ...
  type TriggerMode,
  type TriggerEdge,
} from "@/services/velxio/store/useOscilloscopeStore";
...
import "@/components/velxio/components/simulator/Oscilloscope.css";
...
function getPinsForBoardKind(
  boardKind: BoardKind,
): { pin: number; label: string }[] {
  switch (boardKind) {
    case "arduino-mega":
      return [
...
const ChannelPicker: React.FC<ChannelPickerProps> = ({
  ...
  const pins = selectedBoard
    ? getPinsForBoardKind(selectedBoard.boardKind)
    : [];
    ...
    >
      ...
      {selectedBoard && (
        <div className="osc-picker-board-label">
          {boardDisplayName(selectedBoard)}
        </div>
      )}
    ...
    <div className="osc-container">
      ...
      <div className="osc-header">
        ...
        <button
          ...
          title={
            capturing
              ? t("editor.oscilloscope.pauseTitle")
              : t("editor.oscilloscope.resumeTitle")
          }
        >
          {capturing
            ? `⏸ ${t("editor.oscilloscope.pause")}`
            : `▶ ${t("editor.oscilloscope.run")}`}
        </button>
      ...
      {channels.length === 0 ? (
        <div className="osc-empty">
          ...
          <span style={{ color: "#777" }}>
            {t("editor.oscilloscope.noChannelsHint")}
          </span>
        </div>
      ) : (
        <>
              ...
              <div key={ch.id} className="osc-channel-row">
                <div className="osc-channel-label">
                  ...
                  <button
                    ...
                    title={t("editor.oscilloscope.removeChannel", {
                      label: ch.label,
                    })}
                  >
                    ×

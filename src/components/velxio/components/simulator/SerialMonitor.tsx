...
export const SerialMonitor: React.FC = () => {
  ...
  const clearBoardSerialOutput = useSimulatorStore(
    (s) => s.clearBoardSerialOutput,
  );
  ...
  const [lineEnding, setLineEnding] = useState<"none" | "nl" | "cr" | "both">(
    "nl",
  );
  ...
  const resolvedTabId =
    (boards.find((b) => b.id === activeTabId) ? activeTabId : boards[0]?.id) ??
    null;
          ...
          return (
            <button
              ...
              style={{
                ...styles.tab,
                ...(isActive
                  ? { ...styles.tabActive, borderBottomColor: color, color }
                  : {}),
              }}
      ...
      <pre ref={outputRef} style={styles.output}>
        {activeBoard?.serialOutput
          ? (() => {
              ...
              const text = activeBoard.serialOutput.replace(
                /\x1b\[[0-9;]*m/g,
                "",
              );

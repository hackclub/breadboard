...
import type {
  ...
  WaypointHandle,
  AlignmentGuide,
} from "@/components/velxio/components/simulator/WireLayer";
import { ElectricalOverlay } from "@/components/velxio/components/analog-ui/ElectricalOverlay";
import { BoardOnCanvas } from "@/components/velxio/components/simulator/BoardOnCanvas";
import { CanvasMinimap } from "@/components/velxio/components/simulator/CanvasMinimap";
import { PartSimulationRegistry } from "@/lib/velxio/simulation/parts";
import {
  PROPERTY_CHANGE_EVENT,
  type PropertyChangeDetail,
} from "@/lib/velxio/simulation/parts/partUtils";
import { isSpiceMapped } from "@/lib/velxio/simulation/spice/componentToSpice";
import { PinOverlay } from "@/components/velxio/components/simulator/PinOverlay";
import {
  isBoardComponent,
  boardPinToNumber,
} from "@/lib/velxio/utils/boardPinMapping";
import { autoWireColor, WIRE_KEY_COLORS } from "@/lib/velxio/utils/wireUtils";
import {
  findWireNearPoint,
...
import { isEsp32Family } from "@/lib/velxio/types/boardOptions";
import { BoardOptionsModal } from "@/components/velxio/components/simulator/BoardOptionsModal";
...
} from "@/lib/velxio/utils/analytics";
import { SelectionActionBar } from "@/components/velxio/components/simulator/SelectionActionBar";
import { WireModeBanner } from "@/components/velxio/components/simulator/WireModeBanner";
import { PinPickerDialog } from "@/components/velxio/components/simulator/PinPickerDialog";
import "@/components/velxio/components/simulator/SimulatorCanvas.css";
...
export const SimulatorCanvas = ({
  ...
  const recordRemoveComponent = useSimulatorStore(
    (s) => s.recordRemoveComponent,
  );
  ...
  const [customChipComponentId, setCustomChipComponentId] = useState<
    string | null
  >(null);
  ...
  const [boardOptionsModalFor, setBoardOptionsModalFor] = useState<
    string | null
  >(null);
  ...
  const electricalEngaged = useElectricalStore(
    (s) => s.submittedNetlist !== "",
  );
  ...
  const waypointHandles = React.useMemo<WaypointHandle[]>(() => {
    ...
    const activeDrag =
      waypointDragPreview?.wireId === wire.id ? waypointDragPreview : null;
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      if (interactionRunningRef.current) {
        ...
        if (webComp) {
          ...
          const cid =
            componentWrapper?.getAttribute("data-component-id") || null;
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (touchPassthroughRef.current) {
        ...
        if (pending && e.touches.length === 1) {
          ...
          if (
            dx * dx + dy * dy >
            DRAG_PROMOTE_THRESHOLD_PX * DRAG_PROMOTE_THRESHOLD_PX
          ) {
            const component = componentsRef.current.find(
              (c) => c.id === pending.componentId,
            );
            if (component) {
  ...
  useEffect(() => {
    ...
    const subscribeComponentToPin = (
      ...
      const hasSelfManagedVisuals =
        !!(logic && logic.attachEvents) || spiceOwned;
  ...
  const handleComponentMouseDown = (
    ...
    if (interactionRunning) {
      const component = components.find((c) => c.id === componentId);
      const isSensor =
        !!component && SENSOR_CONTROLS[component.metadataId] !== undefined;
      if (!isSensor) return;
    }
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent) => {
    ...
    if (waypointDragRef.current) {
      ...
      if (wire) {
        ...
        const overridePath = renderedPointsToPath(
          simplifyOrthogonalPath(expanded),
        );
  ...
  const handleCanvasMouseDown = (e: React.MouseEvent) => {
    ...
    const isPanGesture =
      middleOrRight || (leftButton && !wireInProgress && !showPropertyDialog);
  ...
  const renderComponent = (component: any) => {
    ...
    if (
      component.metadataId === "instr-voltmeter" ||
      component.metadataId === "instr-ammeter"
    ) {
      ...
      const showPinsForComponent =
        !dialogOpen &&
        !isTouchDevice &&
        (wireInProgress || isSelected || isHovered);
      return (
        <React.Fragment key={component.id}>
          <div
            ...
            onMouseLeave={() =>
              setHoveredComponentId((curr) =>
                curr === component.id ? null : curr,
              )
            }
    ...
    <div className="simulator-canvas-container">
      ...
      <div className="simulator-canvas">
        {(() => {
          const headerJsx = (
            <div
              className={`canvas-header${headerSlot ? " canvas-header--portaled" : ""}`}
            >
              <div className="canvas-header-left">
                ...
                <span
                  ...
                  title={
                    running
                      ? t("editor.canvas.status.running")
                      : t("editor.canvas.status.stopped")
                  }
                />
              ...
              <div className="canvas-header-right">
                ...
                <span
                  ...
                  title={t("editor.canvas.componentCount", {
          ...
          >
            ...
            {boards.map((board) => {
              ...
              const showPins =
                !dialogOpen &&
                !isTouchDevice &&
                (wireInProgress || isHovered || isActive);
              return (
                <BoardOnCanvas
                  key={board.id}
                  board={board}
                  ...
                  onMouseLeave={() =>
                    setHoveredBoardId((curr) =>
                      curr === board.id ? null : curr,
                    )
                  }
                  onMouseDown={(e) => {
                    setClickStartTime(Date.now());
      ...
      {boardContextMenu &&
        ...
        (() => {
          ...
          const supportsOptions = board
            ? isEsp32Family(board.boardKind)
            : false;
          return (
            <>
              <div
        ...
        (() => {
          ...
          return (
            <BoardOptionsModal
              ...
              onSpiffsChange={(files) =>
                updateBoard(b.id, { spiffsFiles: files })
              }
            />
          );
        })()}
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {connectedWires > 0
                    ? t("editor.canvas.removeConfirm.bodyWithWires", {
                        count: connectedWires,
                      })

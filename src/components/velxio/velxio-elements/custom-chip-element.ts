...
class VelxioCustomChip extends HTMLElement {
  ...
  private _layout(
    ...
    let height = Math.max(
      CHIP_MIN_H,
      CHIP_PAD_Y * 2 + Math.max(0, tallest - 1) * PIN_PITCH,
    );
...
if (
  ...
  !customElements.get("velxio-custom-chip")
) {

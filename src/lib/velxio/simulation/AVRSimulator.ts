...
const MEGA_PIN_TO_PORT = (() => {
  const map: Record<
    number,
    { portName: string; bit: number; port?: AVRIOPort }
  > = {};
...
export class AVRSimulator {
  ...
  private scheduledPinChanges: Array<{
    cycle: number;
    pin: number;
    state: boolean;
  }> = [];
  ...
  constructor(
    ...
    boardVariant: "uno" | "mega" | "tiny85" = "uno",
  ) {
    ...
    const boardName =
      this.boardVariant === "mega"
        ...
        : this.boardVariant === "tiny85"
          ...
          : "ATmega328P";
    console.log(
      `AVR CPU initialized (${boardName}, ${this.peripherals.length} peripherals)`,
    );
  }
        ...
        port.addListener((value) => {
          ...
          if (value !== old) {
            this.pinManager.updatePort(
              ...
              old,
              pinMap,
              ddrAddr ? readDdr(ddrAddr) : undefined,
            );
  ...
  start(): void {
    ...
    if (typeof window !== "undefined") {
      const dbg = (window as unknown as { __spiceDebug?: () => void })
        .__spiceDebug;
      if (typeof dbg === "function") dbg();
      else
        console.warn(
          "[spice] __spiceDebug not attached — startSimulation never called",
        );
    }
    ...
    const execute = (timestamp: number) => {
      ...
      try {
        for (let i = 0; i < cyclesPerFrame; i++) {
          ...
          if (this.scheduledPinChanges.length > 0)
        ...
        if (frameCount % 60 === 0) {
          console.log(
            `[CPU] Frame ${frameCount}, PC: ${this.cpu.pc}, Cycles: ${this.cpu.cycles}`,
          );
        }
      } catch (error) {

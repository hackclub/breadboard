...
const API_BASE = (): string => {
  ...
  if (typeof window !== "undefined") {
    const injected = (window as { __VELXIO_API_BASE__?: string })
      .__VELXIO_API_BASE__;
...
export class Esp32Bridge {
   ...
   */
  onPinChangeWithTime:
    | ((gpioPin: number, state: boolean, timeMs: number) => void)
    | null = null;
  ...
  onWs2812Update: ((channel: number, pixels: Ws2812Pixel[]) => void) | null =
    null;
  /**
    ...
    socket.onopen = () => {
      ...
      this._send({
        ...
        data: {
          ...
          ...(this._pendingFirmware
            ? { firmware_b64: this._pendingFirmware }
            : {}),
    ...
    socket.onmessage = (event: MessageEvent) => {
      ...
      switch (msg.type) {
        case "serial_output": {
          ...
          if (this._pendingMicroPythonCode || this._replState !== "idle") {
            ...
            if (
              ...
              this._serialBuffer.includes('Type "help()"')
            ) {
              this._replState = "banner_seen";
              console.log(
                "[Esp32Bridge] Stage 1: banner seen → poking UART with \\r",
              );
              setTimeout(() => {
                this._send({
            ...
            if (
              this._replState === "prompt_seen" &&
              this._serialBuffer.includes("raw REPL")
            ) {
              ...
              this._serialBuffer = "";
              console.log(
                "[Esp32Bridge] Stage 3: raw REPL confirmed → sending code",
              );
        ...
        case "system": {
          ...
          console.log(
            `[Esp32Bridge:${this.boardId}] system event: ${evt}`,
            msg.data,
          );
          if (evt === "crash") {
        ...
        case "ble_status": {
          ...
          console.log(
            `[Esp32Bridge:${this.boardId}] ble_status: ${bleStatus.status}`,
          );
        ...
        case "error":
          console.error(
            `[Esp32Bridge:${this.boardId}] error: ${msg.data.message as string}`,
          );
  ...
  loadFirmware(firmwareBase64: string): void {
    ...
    if (this._connected) {
      this._send({
        type: "load_firmware",
        data: { firmware_b64: firmwareBase64 },
      });
    }
  }
    ...
    const sendChunk = () => {
      if (offset >= codeBytes.length) {
        ...
        setTimeout(() => {
          ...
          this._replState = "idle";
          console.log(
            `[Esp32Bridge:${this.boardId}] Ctrl+D sent — code executing`,
          );
        }, 300);
        return;
      }

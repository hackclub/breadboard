...
import { RiscVCore } from "@/lib/velxio/simulation/RiscVCore";
import type { PinManager } from "@/lib/velxio/simulation/PinManager";
import { hexToUint8Array } from "@/lib/velxio/utils/hexParser";
import { parseMergedFlashImage } from "@/lib/velxio/utils/esp32ImageParser";
...
export class Esp32C3Simulator {
  ...
  private _registerUart0(): void {
    this.core.addMmio(
      ...
      (addr) => {
        ...
        if (off === UART0_FIFO)
          return this.rxFifo.length > 0 ? this.rxFifo.shift()! & 0xff : 0;
    ...
    this.core.addMmio(
      ...
      (addr, val) => {
        ...
        switch (wordOff) {
          case ST_INT_ENA:
            this._stIntEna =
              (this._stIntEna & ~(0xff << shift)) | ((val & 0xff) << shift);
            break;
          case ST_INT_CLR:
          ...
          default: {
            ...
            peri.set(
              wordAddr,
              (prev & ~(0xff << shift)) | ((val & 0xff) << shift),
            );
            break;
          }
        }
  ...
  private async _loadRom(): Promise<void> {
    try {
      ...
      console.log(
        `[ESP32-C3] ROM binary loaded (${this._romData.length} bytes)`,
      );
    } catch (e) {
      console.warn(
        "[ESP32-C3] Failed to load ROM binary, using C.RET stub:",
        e,
      );
    }
  }
  ...
  private _lowerIntSource(src: number): void {
    ...
    for (let s = 0; s < 62; s++) {
      if (
        s !== src &&
        this._intSrcActive[s] &&
        (this._intSrcMap[s] & 0x1f) === line
      ) {
        stillActive = true;
        break;
      }
    ...
    switch (addr) {
      ...
      case 0x40000358: {
        ...
        for (let i = 0; i < safeN; i++)
          c.writeByte(dest + i, c.readByte(src + i));
        r[10] = dest | 0;
        break;
      }
      case 0x4000035c: {
      ...
      case 0x400007a0: {
        ...
        const a = BigInt.asIntN(
          64,
          BigInt(r[10] >>> 0) | (BigInt(r[11]) << 32n),
        );
        const b = BigInt.asIntN(
          64,
          BigInt(r[12] >>> 0) | (BigInt(r[13]) << 32n),
        );
        r[10] = a < b ? 0 : a === b ? 1 : 2;
        break;
      }
      ...
      case 0x40000764: {
        ...
        const val = BigInt.asIntN(
          64,
          BigInt(r[10] >>> 0) | (BigInt(r[11]) << 32n),
        );
        const abs =
          val < 0n ? BigInt.asUintN(64, -val) : BigInt.asUintN(64, val);
  ...
  start(): void {
    ...
    console.log(
      `[ESP32-C3] Simulation started, entry=0x${this.core.pc.toString(16)}`,
    );
    this.running = true;
    this._loop();
  }

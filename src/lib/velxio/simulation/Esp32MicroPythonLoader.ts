...
const FIRMWARE_MAP: Record<string, FirmwareConfig> = {
  ...
  "esp32-s3": {
    remote:
      "https://micropython.org/resources/firmware/ESP32_GENERIC_S3-20230426-v1.20.0.bin",
    cacheKey: "micropython-esp32s3-v1.20.0",
    fallback: "/firmware/micropython-esp32s3.bin",
  },
  "esp32-c3": {
    remote:
      "https://micropython.org/resources/firmware/ESP32_GENERIC_C3-20230426-v1.20.0.bin",
...
export async function getEsp32Firmware(
  ...
  if (!config)
    throw new Error(`No MicroPython firmware for board: ${boardKind}`);
  ...
  try {
    ...
    if (cached instanceof Uint8Array && cached.length > 0) {
      console.log(
        `[ESP32-MicroPython] Firmware loaded from cache (${variant})`,
      );
      return cached;
    }
  } catch {
 ...
 */
export function padToFlashSize(
  firmware: Uint8Array,
  boardKind?: BoardKind,
): Uint8Array {

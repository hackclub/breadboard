...
import { boardPinToNumber } from "@/lib/velxio/utils/boardPinMapping";
import { classifyPin, isUartWire } from "@/lib/velxio/utils/boardProtocols";
...
function isPi3Bridge(boardKind: string): boolean {
  ...
  return (
    boardKind.startsWith("raspberry-pi-") && boardKind !== "raspberry-pi-pico"
  );
}
...
function buildRouteForWire(wire: Wire): RouteHandle | null {
  ...
  if (isBrowserSim(aEntry.kind)) {
    ...
    teardowns.push(
      installBridgePinFanout(aEntry.id, aRes.pin, bEntry.id, bRes.pin),
    );
  }
  ...
  if (isBrowserSim(bEntry.kind)) {
    ...
    teardowns.push(
      installBridgePinFanout(bEntry.id, bRes.pin, aEntry.id, aRes.pin),
    );
  }
  ...
  if (uartInfo) {
    const aRoleIsTx =
      classifyPin(aEntry.kind, wire.start.pinName).kind === "uart-tx";
...
function collectI2CWirePairs(wires: readonly Wire[]): Map<
  string,
  {
    aBoard: string;
    aBus: number;
 ...
 */
export function notifyBoardReady(
  _boardId: string,
  wires: readonly Wire[],
): void {
  updateI2CBridges(wires);

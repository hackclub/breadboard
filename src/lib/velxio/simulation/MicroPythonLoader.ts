...
export function loadUF2(uf2Data: Uint8Array, flash: Uint8Array): void {
  ...
  for (
    let offset = 0;
    offset + UF2_BLOCK_SIZE <= uf2Data.length;
...
export async function loadUserFiles(
  ...
  const writeFile = lfs.cwrap("lfs_write_file", "number", [
    "number",
    "string",
    "string",
    "number",
...
export async function getFirmware(
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (reader) {
        ...
        console.log(
          `[MicroPython] Firmware downloaded (${firmware.length} bytes)`,
        );
        return firmware;
      }
    }
    ...
    console.warn(
      "[MicroPython] Remote firmware download failed, trying bundled fallback",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      "MicroPython firmware not available (remote and bundled both failed)",
    );
  }
  ...
  console.log(
    `[MicroPython] Firmware loaded from bundled fallback (${firmware.length} bytes)`,
  );
  return firmware;
}

...
import type { Component } from "@/lib/velxio/types/components";
...
export const ACTIVE_DEVICE_PREFIXES: readonly string[] = Object.freeze([
  ...
  "7812",
  "7905",
  "lm317", // regulators (3-terminal active)
]);
...
export function createSpiceResolvedPinResolver(
  ...
  cachedVoltage = voltageSource.getCurrentVoltage(
    componentId,
    componentPinName,
  );
  ...
  return {
    ...
    onChange(cb): () => void {
      return voltageSource.subscribe(
        componentId,
        componentPinName,
        (_state, voltage) => {

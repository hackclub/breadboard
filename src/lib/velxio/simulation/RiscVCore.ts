...
export class RiscVCore {
  ...
  readByte(addr: number): number {
    ...
    if (uAddr >= 0x60000000 && uAddr < 0x80000000) {
      ...
      if (!this._seenUnmapped.has(wordAddr)) {
        ...
        console.warn(
          `[RiscV] unmapped peripheral read @ 0x${wordAddr.toString(16)}`,
        );
      }
    }
    return 0;
  ...
  private decompressC(half: number): number {
    ...
    const sext = (val: number, bits: number) =>
      (val << (32 - bits)) >> (32 - bits);
  ...
  step(): number {
    ...
    if (off0 >= 0 && off0 + 4 <= this.mem.length) {
      ...
      if ((half0 & 0x3) !== 0x3) {
        ...
        instr =
          (half0 | (this.mem[off0 + 2] << 16) | (this.mem[off0 + 3] << 24)) >>>
          0;
        instrLen = 4;
      }
    } else {
      ...
      case 0x73: {
        ...
        if (funct3 === 0) {
          ...
          if (funct12 === 0x302) {
            ...
            nextPc = this.mepc >>> 0;
            if (!oldMie && this.mstatus & 0x8 && this.onMieEnabled)
              this.onMieEnabled();

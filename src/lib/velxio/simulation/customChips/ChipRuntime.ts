 ...
 */
import type { PinManager } from "@/lib/velxio/simulation/PinManager";
import type { I2CBusManager } from "@/lib/velxio/simulation/I2CBusManager";
import { SPIBus, SPIDevice } from "@/lib/velxio/simulation/customChips/SPIBus";
import {
  WasiShim,
  type SimNanosFn,
  type WriteStdoutFn,
} from "@/lib/velxio/simulation/customChips/WasiShim";
import { setChipPinDrive } from "@/lib/velxio/simulation/customChips/chipPinDrives";
import { isSyntheticChipPin } from "@/lib/velxio/simulation/customChips/syntheticPins";
...
export class ChipInstance {
  ...
  tickTimers(nowNanos: bigint | number): void {
    ...
    const table = this.exports?.__indirect_function_table as
      | WebAssembly.Table
      | undefined;
    if (!table) return;
  ...
  private _buildVelxioImports(): Record<string, (...args: any[]) => any> {
    return {
      vx_pin_register: (namePtr: number, mode: number) =>
        this._pin_register(namePtr, mode),
      vx_pin_read: (handle: number) => this._pin_read(handle),
      vx_pin_write: (handle: number, value: number) =>
        this._pin_write(handle, value),
      vx_pin_read_analog: (handle: number) => this._pin_read_analog(handle),
      vx_pin_dac_write: (handle: number, voltage: number) =>
        this._pin_dac_write(handle, voltage),
      vx_pin_set_mode: (handle: number, mode: number) =>
        this._pin_set_mode(handle, mode),
      ...
      vx_attr_register: (namePtr: number, defaultVal: number) =>
        this._attr_register(namePtr, defaultVal),
      ...
      vx_timer_create: (cbIdx: number, ud: number) =>
        this._timer_create(cbIdx, ud),
  ...
  private _syncSpiceDrive(p: PinEntry): void {
    ...
    const isOutput =
      p.mode === ChipInstance.MODE_OUTPUT_LOW ||
      p.mode === ChipInstance.MODE_OUTPUT_HIGH;
  ...
  private _pin_set_mode(handle: number, mode: number): void {
    ...
    if (p.arduinoPin != null) {
      if (mode === ChipInstance.MODE_OUTPUT_LOW)
        this.pinManager.triggerPinChange(p.arduinoPin, false);
      if (mode === ChipInstance.MODE_OUTPUT_HIGH)
        this.pinManager.triggerPinChange(p.arduinoPin, true);
    }
    this._syncSpiceDrive(p);
  }
    ...
    const unsub = this.pinManager.onPinChange(p.arduinoPin, (_pin, state) => {
      ...
      if ((isRising && wantRising) || (isFalling && wantFalling)) {
        const table = this.exports?.__indirect_function_table as
          ...
          | undefined;
        if (!table) return;
        const fn = table.get(cbIdx) as
          | ((ud: number, pin: number, value: number) => void)
          | null;
        if (fn) {
          try {
            fn(userData, handle, newState);
          } catch {
            /* swallow */
          }
        }
  ...
  private _i2c_attach(cfgPtr: number): number {
    if (!this.i2cBus) {
      throw new Error(
        "Chip called vx_i2c_attach but no I2CBusManager is wired to the host",
      );
    }
  ...
  feedUart(byte: number, handle = 0): void {
    ...
    const table = this.exports?.__indirect_function_table as
      | WebAssembly.Table
      | undefined;
    const fn = table?.get(u.on_rx_byte) as
      | ((ud: number, byte: number) => void)
      | null;
    if (fn) {
      try {
        fn(u.user_data, byte & 0xff);
      } catch {
        /* swallow */
      }
    }
  ...
  private _spi_attach(cfgPtr: number): number {
    if (!this.spiBus) {
      throw new Error(
        "Chip called vx_spi_attach but no SPIBus is wired to the host",
      );
    }
    ...
    const onDoneCallback = (_buffer: Uint8Array, count: number) => {
      if (cfg.on_done) {
        const table = this.exports?.__indirect_function_table as
          | WebAssembly.Table
          | undefined;
        const fn = table?.get(cfg.on_done) as
          | ((ud: number, buf: number, c: number) => void)
          | null;
        if (fn) {
          try {
            fn(cfg.user_data, this._currentSpiBufPtr, count);
          } catch {
            /* swallow */
          }
        }
  ...
  private _spi_start(handle: number, bufPtr: number, count: number): void {
    ...
    entry.device.startTransfer(buf, count, (b, c) =>
      entry.onDoneCallback(b, c),
    );
  }
   ...
   *  canvas). */
  onFramebufferUpdate(
    cb: (rgba: Uint8Array, w: number, h: number) => void,
  ): void {

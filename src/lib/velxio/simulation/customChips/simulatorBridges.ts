 ...
 */
import { SPIBus } from "@/lib/velxio/simulation/customChips/SPIBus";
...
export function detectSimulatorKind(simulator: any): SimulatorKind {
  ...
  if (
    ...
    typeof simulator.setSPIHandler === "function"
  ) {
    return "rp2040";
  }
...
export function ensureUartBridge(simulator: any): void {
  ...
  if (kind === "rp2040") {
    ...
    simulator.onSerialData = (charStr: string) => {
      ...
      const code =
        typeof charStr === "string" ? charStr.charCodeAt(0) : Number(charStr);
...
export function avrUartTx(simulator: any, byte: number): void {
  ...
  if (kind === "avr") {
    ...
    if (!simulator.usart || typeof simulator.usart.writeByte !== "function")
      return;
    const b = getSimulatorBridges(simulator);
...
export function ensureSpiBridge(simulator: any): void {
  ...
  if (kind === "rp2040" && typeof simulator.setSPIHandler === "function") {
    ...
    for (const bus of [0, 1] as const) {
      try {
        simulator.setSPIHandler(bus, (mosi: number) =>
          b.spiBus.transferByte(mosi),
        );
      } catch {
        /* this bus may not be in use; ignore */
      }
    }
    b.spiInstalled = true;
    return;

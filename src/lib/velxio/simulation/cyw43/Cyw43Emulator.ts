...
import {
  ...
  WLC_E_STATUS,
  u32le,
} from "@/lib/velxio/simulation/cyw43/constants";
import {
  CDC_HEADER_LEN,
  decodeCdc,
  decodeSdpcm,
  encodeEventFrame,
  encodeSdpcm,
} from "@/lib/velxio/simulation/cyw43/sdpcm";
import {
  bssInfoBlob,
  DEFAULT_AP,
  DEFAULT_STA_MAC,
  type VirtualAp,
} from "@/lib/velxio/simulation/cyw43/virtual-ap";
import type { Cyw43Cmd } from "@/lib/velxio/simulation/cyw43/PioBusSniffer";
...
export class Cyw43Emulator {
  ...
  onConnect = (cb: Listener<ConnectEvent>) =>
    this.add(this.connectListeners, cb);
  onDisconnect = (cb: Listener<DisconnectEvent>) =>
    this.add(this.disconnectListeners, cb);
  onPacketOut = (cb: Listener<PacketOutEvent>) =>
    this.add(this.packetOutListeners, cb);
  ...
  isChipReady(): boolean {
    return this.chipReady;
  }
  getLinkState(): LinkState {
    return this.linkState;
  }
  getStaMac(): Uint8Array {
    return this.staMac;
  }
  ...
  private handleF1(cmd: Cyw43Cmd, payload: Uint8Array): Uint8Array | null {
    if (cmd.write) {
      if (cmd.address === F1.SDIO_BACKPLANE_ADDRESS_LOW) {
        ...
        if (requested & ClockCsr.ALP_AVAIL_REQ)
          this.clockCsr |= ClockCsr.ALP_AVAIL;
        if (requested & ClockCsr.HT_AVAIL_REQ)
          this.clockCsr |= ClockCsr.HT_AVAIL;
      }
...
function safe<T>(cb: (ev: T) => void, ev: T): void {
  try {
    ...
    /* harness — never throw across listener */
  }
}

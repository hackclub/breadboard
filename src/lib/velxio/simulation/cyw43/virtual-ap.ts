...
export const DEFAULT_STA_MAC = new Uint8Array([
...
export function bssInfoBlob(ap: VirtualAp): Uint8Array {
  ...
  let off = 0;
  dv.setUint32(off, 109, true);
  off += 4; // version
  dv.setUint32(off, total, true);
  off += 4; // length
  buf.set(ap.bssid, off);
  off += 6; // bssid
  dv.setUint16(off, 100, true);
  off += 2; // beacon_period (TUs)
  dv.setUint16(off, 0x0421, true);
  off += 2; // capability — ESS + Privacy if secured
  if (!ap.secured) buf[off - 1] &= ~0x10;
  // SSID
  const ssidBytes = new TextEncoder().encode(ap.ssid);
  buf[off] = ssidBytes.length;
  off += 1; // ssid_len
  buf.set(ssidBytes.subarray(0, 32), off);
  off += 32; // ssid (zero-padded)
  // Rates
  ...
  off += 16;
  dv.setUint16(off, ap.channel, true);
  off += 2; // channel
  dv.setUint16(off, 0, true);
  off += 2; // atim_window
  buf[off] = 1;
  off += 1; // dtim_period
  dv.setInt16(off, ap.rssi, true);
  off += 2; // rssi (signed)
  buf[off] = -90;
  off += 1; // phy_noise (signed)
  buf[off] = 1;
  off += 1; // n_cap (HT)

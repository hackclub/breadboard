  ...
  attachEvents: (element, avrSimulator, getArduinoPinHelper) => {
    ...
    const onChange = () => {
      const newValues: number[] =
        (element as any).values || new Array(8).fill(0);
      pins.forEach((pin, i) => {
        if (pin !== null) {
...
PartSimulationRegistry.register("led", {
  attachEvents: (
    ...
    const update = () => {
      ...
      if (
        Date.now() - lastSpiceTs < HOLD_MS &&
        lastSpiceTs > 0 &&
        Number.isFinite(lastSpiceBrightness)
      ) {
...
PartSimulationRegistry.register("membrane-keypad", {
  attachEvents: (element, simulator, getArduinoPinHelper) => {
    ...
    const updateCol = (col: number) => {
      ...
      if (cPin === null) return;
      const colLow = [...activeRows].some((r) =>
        pressedKeys.has(`${r},${col}`),
      );
    ...
    for (let r = 0; r < 4; r++) {
      ...
      const row = r;
      const c = simulator.pinManager.onPinChange(
        rPin,

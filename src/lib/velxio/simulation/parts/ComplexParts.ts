...
import type { AnySimulator } from "@/lib/velxio/simulation/parts/PartSimulationRegistry";
...
PartSimulationRegistry.register("servo", {
  attachEvents: (element, avrSimulator, getArduinoPinHelper) => {
    const pinSIG =
      getArduinoPinHelper("PWM") ??
      getArduinoPinHelper("SIG") ??
      getArduinoPinHelper("1");
    const el = element as any;
    ...
    if (pinSIG !== null && !(avrSimulator instanceof RP2040Simulator)) {
      ...
      const pinManager = (avrSimulator as any).pinManager as
        | import("@/lib/velxio/simulation/PinManager").PinManager
        | undefined;
      ...
      if (pinManager && !hasCpuCycles) {
        ...
        const unsubscribe = pinManager.onPwmChange(
          pinSIG,
          (_pin, dutyCycle) => {
      ...
      if (pinManager) {
        ...
        const unsubscribe = pinManager.onPinChange(pinSIG, (_pin, state) => {
          if (state) {
            ...
            if (pulseUs >= MIN_PULSE_US && pulseUs <= MAX_PULSE_US) {
              const angle = Math.round(
                ((pulseUs - MIN_PULSE_US) / (MAX_PULSE_US - MIN_PULSE_US)) *
                  180,
              );
              el.angle = angle;
            }
    ...
    const poll = () => {
      ...
      if (ocr1a !== lastOcr1a) {
        ...
        const angle = Math.round(
          ((clamped - MIN_PULSE_US) / (MAX_PULSE_US - MIN_PULSE_US)) * 180,
        );
        el.angle = angle;
      }
      ...
      switch (currentCmd) {
        case 0x2a: // CASET – column address set
          if (dataBytes.length === 2)
            colStart = (dataBytes[0] << 8) | dataBytes[1];
        ...
        case 0x2b: // PASET – page address set
          if (dataBytes.length === 2)
            rowStart = (dataBytes[0] << 8) | dataBytes[1];
          if (dataBytes.length === 4) {

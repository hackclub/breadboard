...
PartSimulationRegistry.register("custom-chip", {
  attachEvents: (_element, simulator, getArduinoPin, componentId) => {
    ...
    if (!wasmBase64) {
      console.info(
        `[custom-chip] ${componentId} has no compiled WASM yet — skipping.`,
      );
      return () => {};
    }
    ...
    if (romB64) {
      try {
        ...
        console.warn(
          `[custom-chip] ${componentId} romBytes is not valid base64:`,
          e,
        );
      }
    }
    ...
    if (
      ...
      try {
        ...
        console.error(
          `[custom-chip:${componentId}] failed to register on ESP32 backend:`,
          e,
        );
      }
    ...
    (async () => {
      try {
        ...
        const inst = await ChipInstance.create({
          ...
          romBytes,
          log: (s) =>
            console.log(`[chip:${componentId}] ${s.replace(/\n$/, "")}`),
        });
        if (disposed) {
          inst.dispose();
          return;
        ...
        if (
          el &&
          typeof (el as any).paintFramebuffer === "function" &&
          inst.hasFramebuffer
        ) {
          inst.onFramebufferUpdate((rgba, width, height) => {
            try {
              (el as any).paintFramebuffer(rgba, width, height);
            } catch {
              /* swallow */
            }
          });
        ...
        const tick = () => {
          ...
          if (runnable) {
            try {
              ...
              console.error(
                `[custom-chip:${componentId}] tickTimers threw:`,
                e,
              );
            }
          }

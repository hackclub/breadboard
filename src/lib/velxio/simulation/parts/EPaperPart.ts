...
import {
  ...
  type AnySimulator,
} from "@/lib/velxio/simulation/parts/PartSimulationRegistry";
import {
  SSD168xDecoder,
  type Frame,
} from "@/lib/velxio/simulation/displays/SSD168xDecoder";
import {
  UC8159cDecoder,
  type UC8159cFrame,
  ACEP_PALETTE_RGB,
} from "@/lib/velxio/simulation/displays/UC8159cDecoder";
import { Uc8179Decoder } from "@/lib/velxio/simulation/displays/Uc8179Decoder";
import {
  PANEL_CONFIGS,
  getPanelConfig,
  PANEL_IDS,
} from "@/lib/velxio/simulation/displays/EPaperPanels";
import { RP2040Simulator } from "@/lib/velxio/simulation/RP2040Simulator";
import type { AVRSimulator } from "@/lib/velxio/simulation/AVRSimulator";
...
interface AvrLikeSimulator {
  spi?: {
    ...
    completeTransfer: (resp: number) => void;
  };
  pinManager?: {
    onPinChange(
      pin: number,
      cb: (p: number, state: boolean) => void,
    ): () => void;
  };
}
...
function isEsp32Shim(sim: AnySimulator): sim is Esp32LikeSimulator {
  ...
  return (
    typeof s.getBridge === "function" && typeof s.registerSensor === "function"
  );
}
...
const epaperSimulation = {
  attachEvents: (
    ...
    const panelKind =
      (element.getAttribute && element.getAttribute("panel-kind")) ??
      "epaper-1in54-bw";
    const cfg = getPanelConfig(panelKind);
    const explicitRefreshMs = parseFloat(
      element.getAttribute("refresh-ms") ?? "",
    );
    const refreshMs =
      !isNaN(explicitRefreshMs) && explicitRefreshMs > 0
        ? explicitRefreshMs
        : cfg.refreshMs;
    ...
    const scheduleFlush = (frame: AnyFrame) => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (ctx) {
          if (cfg.palette === "acep")
            paintAcePFrame(ctx, pendingFrame as UC8159cFrame);
        ...
        const rp = (sim as any).rp2040 as
          | {
              spi: Array<{
                ...
                completeTransmit: (v: number) => void;
              }>;
            }
          | undefined;
        if (rp?.spi?.length) {

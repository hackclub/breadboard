...
import type { PartSimulationLogic } from "@/lib/velxio/simulation/parts/PartSimulationRegistry";
  ...
  return {
    attachEvents: (
      ...
      if (useResolver) {
        ...
        if (clkPin === null || dataPinIds.some((p) => p === null))
          return () => {};
        unsubs.push(
...
PartSimulationRegistry.register(
  "logic-gate-and-3",
  nInputGate(["A", "B", "C"], allTrue),
);
PartSimulationRegistry.register(
  "logic-gate-or-3",
  nInputGate(["A", "B", "C"], anyTrue),
);
PartSimulationRegistry.register(
  "logic-gate-nand-3",
  nInputGate(["A", "B", "C"], notAll),
);
PartSimulationRegistry.register(
  "logic-gate-nor-3",
  nInputGate(["A", "B", "C"], notAny),
);
PartSimulationRegistry.register(
  "logic-gate-and-4",
  nInputGate(["A", "B", "C", "D"], allTrue),
);
PartSimulationRegistry.register(
  "logic-gate-or-4",
  nInputGate(["A", "B", "C", "D"], anyTrue),
);
PartSimulationRegistry.register(
  "logic-gate-nand-4",
  nInputGate(["A", "B", "C", "D"], notAll),
);
PartSimulationRegistry.register(
  "logic-gate-nor-4",
  nInputGate(["A", "B", "C", "D"], notAny),
);
...
PartSimulationRegistry.register("logic-gate-not", {
  attachEvents: (element, simulator, getPin, _componentId, getPinResolver) => {
    ...
    const unsub = simulator.pinManager.onPinChange(
      pinA,
      (_: number, s: boolean) => {

...
import { HD44780Decoder } from "@/lib/velxio/simulation/HD44780Decoder";
import {
  ...
  attachEvents: (element, simulator, _getPin, componentId) => {
    ...
    if (typeof sim.registerSensor === "function") {
      ...
      registerSensorUpdate(componentId, (values) => {
        ...
        if ("accelX" in values)
          writeI16(0x3b, (values.accelX as number) * 16384);
        if ("accelY" in values)
          writeI16(0x3d, (values.accelY as number) * 16384);
        if ("accelZ" in values)
          writeI16(0x3f, (values.accelZ as number) * 16384);
        ...
        if ("temp" in values)
          writeI16(0x41, ((values.temp as number) - 36.53) * 340);
      });
...
PartSimulationRegistry.register("bmp280", {
  attachEvents: (element, simulator, _getPin, componentId) => {
    ...
    const initTemp =
      el.temperature !== undefined ? parseFloat(el.temperature) : 25.0;
    const initPressure =
      el.pressure !== undefined ? parseFloat(el.pressure) : 1013.25;
    ...
    if (typeof sim.registerSensor === "function") {
      ...
      registerSensorUpdate(componentId, (values) => {
        sim.updateSensor(virtualPin, values);
        if ("temperature" in values)
          dev.temperatureC = values.temperature as number;
...
PartSimulationRegistry.register("ds3231", {
  attachEvents: (element, simulator, _getPin, componentId) => {
    ...
    if (typeof sim.registerSensor === "function") {
      ...
      dev.temperatureC = initTemp;
      sim.registerSensor("ds3231", virtualPin, {
        addr: 0x68,
        temperature: initTemp,
      });
      sim.addI2CDevice?.(dev);

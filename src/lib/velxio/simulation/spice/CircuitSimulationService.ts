...
export class CircuitSimulationService {
  ...
  /**
   * Process a single MCU pin change notification.
   */
  async handleMcuEdge(
    boardId: string,
    pinName: string,
    ...
    const expected =
      `v_${sanitizeSpiceId(boardId)}_${sanitizeSpiceId(pinName)}`.toLowerCase();
  ...
  private async runSolve(): Promise<void> {
    ...
    for (const vs of voltageSources)
      extraVectors.push(`i(${vs.toLowerCase()})`);
    ...
    if (input.analysis.kind === "tran") {
      await this.scheduler.resolveTran(
        input.analysis.step,
        input.analysis.stop,
      );
    } else {
  ...
  private publishFromLastResult(): void {
    ...
    if (ctx.analysisKind === "tran" && result.timeAxis.length > 0) {
      ...
      for (const vs of ctx.voltageSources) {
        const vec = result.vectors.get(`i(${vs.toLowerCase()})`);
        if (vec && vec.real.length > 0)
          branches.set(vs.toLowerCase(), Array.from(vec.real));
      }

...
import type {
  SolverPort,
  SolveAnalysis,
} from "@/lib/velxio/simulation/spice/ports/SolverPort";
import { NgSpiceWorkerAdapter } from "@/lib/velxio/simulation/spice/adapters/NgSpiceWorkerAdapter";
import { sanitizeSpiceId } from "@/lib/velxio/simulation/spice/NetlistBuilder";
import type {
  PinState,
  SpiceVoltageSource,
...
class MixedModeSchedulerImpl implements SpiceVoltageSource {
  ...
  async resolveTran(step: string, stop: string): Promise<void> {
    if (!this.solver) {
      throw new Error(
        "MixedModeScheduler.resolveTran(): call loadCircuit first",
      );
    }
  ...
  private lastResult:
    | import("@/lib/velxio/simulation/spice/ports/SolverPort").SolveResult
    | null = null;
  ...
  publishVoltage(
    ...
    for (const sub of this.subscriptions.values()) {
      if (
        ...
        sub.componentPinName === componentPinName
      ) {
 ...
 */
export function __setSchedulerSolverFactoryForTests(
  factory: () => SolverPort,
): void {

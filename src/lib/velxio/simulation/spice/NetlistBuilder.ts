...
export function buildNetlist(input: BuildNetlistInput): BuildNetlistResult {
  ...
  const pinKey = (componentId: string, pinName: string) =>
    `${componentId}:${pinName}`;
  ...
  for (const comp of components) {
    for (const group of getBreadboardConnectionGroups(comp.metadataId)) {
      const referencedPins = group
        .map((pinName) => pinKey(comp.id, pinName))
        .filter((key) => uf.has(key));
      const [first, ...rest] = referencedPins;
      if (!first) continue;
      uf.add(first);
...
function pinsReferencedByWires(
  componentId: string,
  wires: WireForSpice[],
): string[] {
...
export function buildWireNetMap(
  ...
  for (const comp of components) {
    ...
    for (const pName of pinsReferencedByWires(comp.id, wires)) {
      ...
      else if (VCC_PIN_RE.test(pName))
        uf.setCanonical(pin(comp.id, pName), "vcc_rail");
    }
  }

...
export type { SolveVector } from "@/lib/velxio/simulation/spice/ports/SolverPort";
import {
  loadNgSpiceForNode,
  type NgSpiceEmscriptenModule,
} from "@/lib/velxio/simulation/spice/adapters/node/loadNgSpiceForNode";
...
export class NgSpiceNodeAdapter implements SolverPort {
  ...
  async loadCircuit(netlist: string): Promise<void> {
    await this.init();
    if (!this.module || !this.api || !this.module._velxio_fs)
  ...
  async solve(
    ...
    return {
      ...
      warnings: [
        ...this.stderrBuffer,
        ...this.stdoutBuffer.map((s) => `stdout: ${s}`),
      ],
    };
  }
  ...
  private readVector(name: string): SolveVector | null {
    ...
    const interleaved = new Float64Array(
      heapf64.buffer,
      compDataPtr,
      length * 2,
    );

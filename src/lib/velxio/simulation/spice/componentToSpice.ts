...
const MAPPERS: Record<string, Mapper> = {
  ...
  "power-supply": (comp, netLookup) => {
    ...
    const currentLimit = Math.max(
      0.01,
      Number(comp.properties.currentLimit ?? 1),
    );
  ...
  potentiometer: (comp, netLookup) => {
    ...
    const total = parseValueWithUnits(
      comp.properties.total ?? comp.properties.resistance,
      10_000,
    );
  ...
  "slide-potentiometer": (comp, netLookup) => {
    ...
    const pos =
      Number(comp.properties.position ?? comp.properties.percent ?? 50) / 100;
  ...
  ...(() => {
    ...
    function ic2InputQuad(
      ...
      return (comp, netLookup, ctx) => {
        ...
        for (let i = 1; i <= 4; i++) {
          ...
          cards.push(
            `B_${comp.id}_${i} ${y} 0 V = ${build(a, b, y, T, ctx.vcc)}`,
          );
    ...
    return {
      ...
      "ic-74hc08": ic2InputQuad(
        (a, b, _y, T, vcc) => `${vcc} * u(V(${a})-${T}) * u(V(${b})-${T})`,
      ),
      ...
      // 74HC04 — hex inverter
      "ic-74hc04": ic1InputHex(
        (a, _y, T, vcc) => `${vcc} * (1 - u(V(${a})-${T}))`,
      ),
  ...
  photoresistor: (comp, netLookup) => {
    ...
    const pins =
      twoPin(comp, netLookup, "LDR1", "LDR2") ??
...
export const PASSIVE_PRESETS: Readonly<
  Record<
    string,
    "resistor" | "capacitor" | "capacitor-electrolytic" | "inductor"
  >
> = {

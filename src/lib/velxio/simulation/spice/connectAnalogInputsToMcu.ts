...
import { interpolateAt } from "@/lib/velxio/simulation/spice/waveformStats";
...
const ADC_PIN_MAP: Partial<
  Record<BoardKind, Array<{ pinName: string; channel: number }>>
> = {
  // AVR
  "arduino-uno": ADC_6CH,
...
const ADC_PIN_TO_GPIO: Partial<
  Record<BoardKind, (pinName: string, channel: number) => number>
> = {
...
export function connectAnalogInputsToMcu(): () => void {
  ...
  function installAdcReadHooks() {
    ...
    for (const board of boards) {
      ...
      if (!sim) continue;
      const adc = (
        sim as unknown as { getADC?: () => object | null }
      ).getADC?.();
      ...
      self.onADCRead = function (input) {
        ...
        switch (input.type) {
          ...
          case 1: {
            ...
            const vPos =
              sampleWaveformAtNow(channelToNet.get(pos) ?? "") ??
              self.channelValues[pos] ??
              0;
            const vNeg =
              sampleWaveformAtNow(channelToNet.get(neg) ?? "") ??
              self.channelValues[neg] ??
              0;
        ...
        self.cpu.addClockEvent(
          () => self.completeADCRead(result),
          self.sampleCycles,
        );
      };
      patchedAdcs.add(adc);
  ...
  const unsubResult = useElectricalStore.subscribe((state, prev) => {
    if (
      state.nodeVoltages !== prev.nodeVoltages ||

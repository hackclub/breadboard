...
import type { CircuitSimulationService } from "@/lib/velxio/simulation/spice/CircuitSimulationService";
 ...
 */
export function connectMcuEdgesToService(
  service: CircuitSimulationService,
): () => void {
  ...
  function schedulePin(
    ...
    const timer = setTimeout(
      () => flushPin(boardId, pinName),
      COALESCE_WINDOW_MS,
    );
    pending.set(key, { state, vcc, pinName, timer });
  }

  function arduinoPinToName(
    arduinoPin: number,
    boardKind: string,
  ): string | null {
  ...
  function subscribeBoard(boardId: string, boardKind: string): void {
    ...
    if (!pm) return;
    const group =
      BOARD_PIN_GROUPS[boardKind as keyof typeof BOARD_PIN_GROUPS] ??
      BOARD_PIN_GROUPS.default;
    const vcc = group.vcc;

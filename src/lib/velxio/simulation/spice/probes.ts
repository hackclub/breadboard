...
import { UnionFind } from "@/lib/velxio/simulation/spice/unionFind";
import {
  rms,
  mean,
  peak,
  subtract,
  isAC,
} from "@/lib/velxio/simulation/spice/waveformStats";
...
function formatV(v: number): {
  ...
  if (abs < 1e-6)
    return {
      value: v * 1e6,
      unit: "µV",
      display: `${(v * 1e6).toFixed(2)} µV`,
    };
  if (abs < 1)
    return {
      value: v * 1e3,
      unit: "mV",
      display: `${(v * 1e3).toFixed(2)} mV`,
    };
  return { value: v, unit: "V", display: `${v.toFixed(3)} V` };
}

function formatI(i: number): {
  value: number;
  unit: ProbeReading["unit"];
  display: string;
} {
  ...
  if (abs < 1e-6)
    return {
      value: i * 1e9,
      unit: "nA",
      display: `${(i * 1e9).toFixed(1)} nA`,
    };
  if (abs < 1e-3)
    return {
      value: i * 1e6,
      unit: "µA",
      display: `${(i * 1e6).toFixed(2)} µA`,
    };
  if (abs < 1)
    return {
      value: i * 1e3,
      unit: "mA",
      display: `${(i * 1e3).toFixed(2)} mA`,
    };
...
export function buildPinNetLookup(
  ...
  for (const g of groundPins)
    uf.setCanonical(`${g.componentId}:${g.pinName}`, "0");
  for (const v of vccPins)
    uf.setCanonical(`${v.componentId}:${v.pinName}`, "vcc_rail");
...
export function readVoltmeter(
  ...
  if (timeWaveforms) {
    ...
    const minusSamples =
      plusNet === "0" ? undefined : timeWaveforms.nodes.get(minusNet);

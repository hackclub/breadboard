 ...
 */
import type { SolverPort } from "@/lib/velxio/simulation/spice/ports/SolverPort";
...
interface AdapterWithRead {
  readAllCurrentVectors():
    | Promise<{
        vectors: Map<
          string,
          import("@/lib/velxio/simulation/spice/ports/SolverPort").SolveVector
        >;
        rawNames: string[];
      }>
    | {
...
export async function runNetlist(netlist: string): Promise<SpiceResult> {
  ...
  const all = await (
    adapter as unknown as AdapterWithRead
  ).readAllCurrentVectors();
  const result = {
    analysis,
    vectors: all.vectors,
  ...
  const getVec = (name: string): VectorValue[] => {
    ...
    const vec =
      result.vectors.get(ngKey) ?? result.vectors.get(name.toLowerCase());
    if (!vec) {
      throw new Error(

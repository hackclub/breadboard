...
async function runSimulation(requestId, netlist) {
  ...
  postDebug("run-start", {
    ...
    netlistLines: netlist.split(/\r?\n/).length,
  });

  self.postMessage({
    type: "status",
    requestId,
    message: "Submitting circuit to shared ngspice…",
  });
...
function extractAllAnalyses() {
  ...
  postDebug("extract-analyses", {
    plotCount: plotNames.length,
...
async function ensureSession(config) {
  ...
  if (!filesystemReady) {
    self.postMessage({
      ...
      message: "Staging library assets…",
    });
...
async function ensureModule(config) {
  if (!moduleReady) {
    ...
    moduleReady = new Promise((resolve, reject) => {
      ...
      try {
        importScripts(
          resolveAssetUrl(moduleConfig.assetBaseUrl, moduleConfig.moduleScript),
        );
      } catch (error) {
        reject(error);
      }
...
function bindApi() {
  api = {
    init: Module.cwrap("ngSpice_Init", "number", [
      "number",
      "number",
      "number",
      "number",
...
function onPrint(messagePtr) {
  ...
  if (activeCommandCapture) {
    ...
    self.postMessage({
      type: channel,
      requestId: activeCommandCapture.requestId,
      line: body,
    });
    return 0;
  }
...
function onDataInit(vecinfoAllPtr) {
  ...
  if (currentRun) {
    self.postMessage({
      type: "status",
      requestId: currentRun.requestId,
      message: "Transient vectors initialized.",
    });
  }
  return 0;
}
...
function emitProgress(force, currentTimeOverride = null) {
  ...
  const currentTime =
    currentTimeOverride ??
    readLatestVectorValue(currentRun.timeVectorName || "time");
  if (currentTime === null) {
    return;
  }
  ...
  const progress = Math.min(
    Math.max(currentTime / currentRun.finalTime, currentRun.lastProgress),
    1,
  );
  if (
    !force &&
    progress - currentRun.lastProgress < 0.001 &&
    now - currentRun.lastEmitAt < 80
  ) {
    return;
  }
...
function readLatestVectorValue(vectorName) {
  ...
  const realDataPtr =
    HEAPU32[(vectorInfoPtr + VECTOR_INFO_REALDATA_OFFSET) >> 2];
...
function readCurrentTimeFromData(vecvaluesAllPtr, vectorCount) {
  ...
  const count =
    vectorCount || HEAP32[(vecvaluesAllPtr + VECVALUESALL_COUNT_OFFSET) >> 2];
  const valuesPtr =
    HEAPU32[(vecvaluesAllPtr + VECVALUESALL_VALUES_OFFSET) >> 2];
...
function resolveAssetUrl(basePath, fileName) {
  return new URL(
    `${trimTrailingSlash(basePath)}/${fileName}`,
    self.location.href,
  ).toString();
}
...
async function fetchBinary(fileName) {
  const response = await fetch(
    resolveAssetUrl(moduleConfig.assetBaseUrl, fileName),
  );
  if (!response.ok) {
    throw new Error(

...
export function isBoardComponent(componentId: string): boolean {
  return BOARD_COMPONENT_IDS.some(
    (id) => componentId === id || componentId.startsWith(id),
  );
}
 ...
 */
export function boardPinToNumber(
  boardId: string,
  pinName: string,
): number | null {
  if (boardId === "arduino-uno" || boardId === "arduino-nano") {
    // Power / GND pins — not real GPIOs, skip silently
    if (/^(GND|VCC|VIN|IOREF|AREF|RESET|3\.3V|3V3|5V|3V)/.test(pinName))
      return -1;
  ...
  if (
    boardId === "raspberry-pi-3" ||
    boardId.startsWith("raspberry-pi-3") ||
    boardId === "raspberry-pi-4" ||
    boardId.startsWith("raspberry-pi-4") ||
    boardId === "raspberry-pi-5" ||
    boardId.startsWith("raspberry-pi-5")
  ) {
  ...
  if (boardId === "riscv-generic" || boardId.startsWith("riscv-generic")) {
    ...
    if (/^PC(\d+)$/.test(pinName))
      return 8 + parseInt(pinName.substring(2), 10);
    if (/^PD(\d+)$/.test(pinName))
      return 16 + parseInt(pinName.substring(2), 10);
    // Numeric fallback
    const num = parseInt(pinName, 10);

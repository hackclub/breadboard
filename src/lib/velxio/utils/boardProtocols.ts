...
function tableFor(boardKind: BoardKind | string): RoleTable | null {
  if (
    ...
    (boardKind as string).startsWith("stm32-")
  )
    return STM32_DEFAULT;
  ...
  if (boardKind === "raspberry-pi-pico" || boardKind === "pi-pico-w")
    return RP2040_DEFAULT;
  if (boardKind === "esp32-c3" || (boardKind as string).startsWith("esp32-c3"))
    return ESP32_C3_DEFAULT;
  if (boardKind === "esp32" || (boardKind as string).startsWith("esp32"))
    return ESP32_DEFAULT;
  // Pi Zero/1/2/3/4/5 all share the same 40-pin GPIO header → same BCM table.
  if ((boardKind as string).startsWith("raspberry-pi-")) return PI3_BCM;
...
export function isUartWire(
  ...
  if (ra.kind === "uart-tx" && rb.kind === "uart-rx")
    return { uartA: ra.uart, uartB: rb.uart };
  if (ra.kind === "uart-rx" && rb.kind === "uart-tx")
    return { uartA: ra.uart, uartB: rb.uart };
  return null;
}

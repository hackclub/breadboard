...
export function detectFirmwareFormat(
  filename: string,
  bytes: Uint8Array,
): FirmwareFormat {
  // Check ELF magic
...
export function extractLoadSegmentsFromElf(bytes: Uint8Array): Uint8Array {
  ...
  const u16 = (off: number) =>
    isLE ? view.getUint16(off, true) : view.getUint16(off, false);
  const u32 = (off: number) =>
    isLE ? view.getUint32(off, true) : view.getUint32(off, false);
  ...
  for (let i = 0; i < e_phnum; i++) {
    ...
    if (p_offset + p_filesz > bytes.length) {
      throw new Error(
        `ELF segment at offset 0x${p_offset.toString(16)} extends beyond file`,
      );
    }
...
function _isRp2040Board(kind: BoardKind): boolean {

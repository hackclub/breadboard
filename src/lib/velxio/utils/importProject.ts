...
import { importVlxFile, VlxParseError } from "@/lib/velxio/utils/vlxFile";
import {
  importFromWokwiZip,
  type VelxioComponent,
} from "@/lib/velxio/utils/wokwiZip";
...
export async function importProjectFile(
  ...
  if (
    lower.endsWith(".vlx") ||
    (file.type === "application/json" && !lower.endsWith(".zip"))
  ) {
    try {
      ...
      const msg =
        err instanceof VlxParseError ? err.message : (err as Error).message;

...
import { isPiBoardKind } from "@/lib/velxio/types/board";
import {
  useEditorStore,
  chipFileGroupId,
  CHIP_GROUP_PREFIX,
...
import { isBoardComponent } from "@/lib/velxio/utils/boardPinMapping";
import {
  getInstalledLibraries,
  installLibrary,
} from "@/services/velxio/services/libraryService";
import { trackOpenExample } from "@/lib/velxio/utils/analytics";
import { stripBrandPrefix } from "@/lib/velxio/utils/exampleToBuildNetlistInput";
  ...
  if (example.boards && example.boards.length > 0) {
    ...
    example.boards.forEach((eb, idx) => {
      ...
      if (eb.code) {
        ...
        const filename = isPiBoardKind(eb.boardKind)
          ? "main.cpp"
          : "sketch.ino";
        useEditorStore.getState().setActiveGroup(board.activeFileGroupId);
        useEditorStore
          .getState()
          .loadFiles([{ name: filename, content: eb.code }]);
      }
    ...
    if (liveBoard) {
      ...
      const groupId =
        updatedBoard?.activeFileGroupId ?? liveBoard.activeFileGroupId;

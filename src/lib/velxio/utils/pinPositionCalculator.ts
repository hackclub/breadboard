...
export function calculatePinPosition(
  ...
  if (!element) {
    ...
    if (process.env.NODE_ENV !== "test") {
      console.warn(
        `[pinPositionCalculator] Component ${componentId} not found in DOM`,
      );
    }
    return null;
  }
  ...
  if (!pinInfo || !Array.isArray(pinInfo)) {
    if (process.env.NODE_ENV !== "test") {
      console.warn(
        `[pinPositionCalculator] Component ${componentId} does not have pinInfo`,
      );
    }
    return null;
  }
  ...
  if (!pin) {
    console.warn(
      `[pinPositionCalculator] Pin ${pinName} not found on component ${componentId}`,
    );
    console.warn(
      `Available pins:`,

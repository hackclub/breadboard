...
export async function adminGetBoardDiversity(): Promise<BoardDiversityResponse> {
  const { data } = await api.get<BoardDiversityResponse>(
    "/admin/metrics/board-diversity",
  );
  return data;
}
...
export async function adminGetTopProjects(
  ...
  const { data } = await api.get<TopProjectEntry[]>(
    "/admin/metrics/top-projects",
    {
      params: { metric, limit },
    },
  );
...
export async function adminGetUserMetrics(
  ...
  const { data } = await api.get<UserMetricsResponse>(
...
export async function adminGetProjectMetrics(
  ...
  const { data } = await api.get<ProjectMetricsResponse>(
    `/admin/metrics/projects/${projectId}`,
    {
      params: { range_days: rangeDays },
    },
  );

...
import type { CompilationLog } from "@/lib/velxio/utils/compilationLogger";
...
export const useCompileLogsStore = create<CompileLogsState>((set, get) => ({
  ...
  setLogs: (next) => {
    if (typeof next === "function") {
      set({
        logs: (next as (prev: CompilationLog[]) => CompilationLog[])(
          get().logs,
        ),
      });
    } else {
  ...
  appendLogs: (entries) => set((s) => ({ logs: [...s.logs, ...entries] })),
  clear: () => set({ logs: [] }),
}));

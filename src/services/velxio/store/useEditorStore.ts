 ...
 */
export const chipFileGroupId = (chipId: string): string =>
  `group-chip-${chipId}`;
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  deleteFile: (id: string) => {
    set((s) => {
      ...
      const groupFiles = (s.fileGroups[groupId] ?? []).filter(
        (f) => f.id !== id,
      );
      const groupOpenIds = (s.openGroupFileIds[groupId] ?? []).filter(
        (fid) => fid !== id,
      );
      return {
        files,
  ...
  renameFile: (id: string, newName: string) => {
    set((s) => {
      ...
      return {
        ...
        fileGroups: {
          ...s.fileGroups,
          [groupId]: (s.fileGroups[groupId] ?? []).map(mapper),
        },
      };
    });
  },
  ...
  openFile: (id: string) => {
    set((s) => {
      ...
      return {
        ...
        openGroupFileIds: {
          ...s.openGroupFileIds,
          [groupId]: groupOpenIds.includes(id)
            ? groupOpenIds
            : [...groupOpenIds, id],
        },
        activeGroupFileId: { ...s.activeGroupFileId, [groupId]: id },
  ...
  createFileGroup: (
    ...
    set((s) => {
      ...
      const initialFiles = Array.isArray(languageModeOrFiles)
        ? languageModeOrFiles
        : undefined;
      const languageMode =
        typeof languageModeOrFiles === "string"
          ? languageModeOrFiles
          : undefined;
  ...
  setActiveGroup: (groupId: string) => {
    set((s) => {
      ...
      const activeFileId =
        s.activeGroupFileId[groupId] ?? groupFiles[0]?.id ?? "";
      const openFileIds =
        s.openGroupFileIds[groupId] ??
        (groupFiles[0] ? [groupFiles[0].id] : []);
      return {

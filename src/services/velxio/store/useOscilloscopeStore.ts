 ...
 */
export function matchesTriggerEdge(
  prevState: boolean,
  newState: boolean,
  edge: TriggerEdge,
): boolean {
...
export const useOscilloscopeStore = create<OscilloscopeState>((set, get) => ({
  ...
  pushSample: (channelId, timeMs, state) => {
    ...
    if (s.triggerMode !== "auto") {
      const triggerChId = resolveTriggerChannelId(
        s.triggerChannelId,
        s.channels,
      );
      if (triggerChId === channelId) {
        ...
        if (triggerBuf && triggerBuf.length > 0) {
          ...
          if (
            !captureLocked &&
            matchesTriggerEdge(prevState, state, s.triggerEdge)
          ) {
    ...
    set((cur) => {
      ...
      return {
        ...
        ...(nextTriggeredAtMs !== cur.triggeredAtMs
          ? { triggeredAtMs: nextTriggeredAtMs }
          : {}),
        ...(nextRunning !== cur.running ? { running: nextRunning } : {}),
        ...(nextStatus !== cur.triggerStatus

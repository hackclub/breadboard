...
import { Cyw43Bridge } from "@/lib/velxio/simulation/cyw43";
...
import {
  ...
  I2CBusManager,
  nullI2CMaster,
} from "@/lib/velxio/simulation/I2CBusManager";
import type { I2CDevice } from "@/lib/velxio/simulation/I2CBusManager";
import type { RP2040I2CDevice } from "@/lib/velxio/simulation/RP2040Simulator";
import type {
  Wire,
  WireInProgress,
  WireEndpoint,
} from "@/lib/velxio/types/wire";
import type {
  BoardKind,
  BoardInstance,
  LanguageMode,
...
import {
  Stm32Bridge,
  stm32PinNameToLinear,
} from "@/lib/velxio/simulation/Stm32Bridge";
import { STM32_LED } from "@/components/velxio/components/velxio-components/Stm32BluePillElement";
import { useEditorStore } from "@/services/velxio/store/useEditorStore";
import { useVfsStore } from "@/services/velxio/store/useVfsStore";
import {
  boardPinToNumber,
  isBoardComponent,
...
import {
  autoWireColor,
  DEFAULT_WIRE_COLOR,
} from "@/lib/velxio/utils/wireUtils";
import { createSerialBatcher } from "@/services/velxio/store/serialBatcher";
import {
...
class Esp32BridgeShim {
  ...
  onPinChangeWithTime:
    | ((pin: number, state: boolean, timeMs: number) => void)
    | null = null;
  ...
  private _i2cTransactionListeners = new Map<
    number,
    (data: number[]) => void
  >();
...
function makeGpioRoutingHandler(boardId: string) {
  return (routing: { gpio: number; signal_id: number }) => {
    signalRouterMap
      .get(boardId)
      ?.updateRouting(routing.gpio, routing.signal_id);
  };
}
  ...
  function getOscilloscopeCallback(boardId: string) {
    return (pin: number, state: boolean, timeMs: number) => {
      ...
      for (const ch of channels) {
        if (ch.boardId === boardId && ch.pin === pin)
  ...
  return {
      ...
      if (isPiBoardKind(boardKind)) {
        ...
        bridge.onPinChange = (gpioPin, state) => {
          ...
          if (gpioPin === ledLinear) {
            const dom = document.getElementById(id) as
              | (HTMLElement & { led?: boolean })
              | null;
            if (dom && "led" in dom)
              dom.led = ledCfg.activeLow ? !state : !!state;
          }
        };
        bridge.onPinChangeWithTime = getOscilloscopeCallback(id);
        bridge.onDisconnected = () => {
    ...
    setActiveBoardId: (boardId: string) => {
      ...
      set({
        ...
        remoteConnected:
          bridgeMap.get(boardId)?.connected ??
          esp32BridgeMap.get(boardId)?.connected ??
          false,
        remoteSocket: null,
      });
    ...
    compileBoardProgram: (boardId: string, program: string) => {
      ...
      if (isEsp32Kind(board.boardKind)) {
        ...
        if (sim instanceof Esp32C3Simulator) {
          try {
            ...
            console.error(
              `[Esp32C3Simulator] loadFlashImage failed for ${boardId}:`,
              err,
            );
            return;
          }
      ...
      set((s) => {
        ...
        return {
          boards,
          ...(isActive
            ? { compiledHex: program, hexEpoch: s.hexEpoch + 1 }
            : {}),
        };
      });
    },
    ...
    loadMicroPythonProgram: async (
      ...
      set((s) => {
        const boards = s.boards.map((b) =>
          b.id === boardId
            ? { ...b, compiledProgram: "micropython-loaded" }
            : b,
        );
        const isActive = s.activeBoardId === boardId;
        return {
          boards,
          ...(isActive
            ? { compiledHex: "micropython-loaded", hexEpoch: s.hexEpoch + 1 }
            : {}),
        };
      });
    },
    ...
    setBoardLanguageMode: (boardId: string, mode: LanguageMode) => {
      ...
      if (
        mode === "micropython" &&
        !BOARD_SUPPORTS_MICROPYTHON.has(board.boardKind)
      )
        return;
      ...
      set((s) => ({
        boards: s.boards.map((b) =>
          b.id === boardId
            ? { ...b, languageMode: mode, compiledProgram: null }
            : b,
        ),
      }));
    ...
    startBoard: (boardId: string) => {
        ...
        if (esp32Bridge) {
          ...
          for (const comp of components) {
            ...
            for (const w of wires) {
              const compEndpoint =
                w.start.componentId === comp.id &&
                w.start.pinName === sensorDef.dataPinName
                  ? w.start
                  : w.end.componentId === comp.id &&
                      w.end.pinName === sensorDef.dataPinName
                    ? w.end
                    : null;
              ...
              const gpioPin = boardPinToNumber(
                board.boardKind,
                boardEndpoint.pinName,
              );
              ...
              for (const key of sensorDef.propertyKeys) {
                ...
                if (val !== undefined)
                  props[key] = typeof val === "string" ? parseFloat(val) : val;
              }
              // Resolve extra pins (e.g. echo_pin for HC-SR04) from wires
              if (sensorDef.extraPins) {
                for (const [propName, compPinName] of Object.entries(
                  sensorDef.extraPins,
                )) {
                  for (const ew of wires) {
                    const epComp =
                      ew.start.componentId === comp.id &&
                      ew.start.pinName === compPinName
                        ? ew.start
                        : ew.end.componentId === comp.id &&
                            ew.end.pinName === compPinName
                          ? ew.end
                          : null;
                    if (!epComp) continue;
                    const epBoard = epComp === ew.start ? ew.end : ew.start;
                    if (!isBoardComponent(epBoard.componentId)) continue;
                    const extraGpio = boardPinToNumber(
                      board.boardKind,
                      epBoard.pinName,
                    );
      ...
      set((s) => {
        const boards = s.boards.map((b) =>
          b.id === boardId
            ? { ...b, running: true, serialMonitorOpen: true }
            : b,
        );
        const isActive = s.activeBoardId === boardId;
        return {
    ...
    resetBoard: (boardId: string) => {
      ...
      set((s) => {
        const boards = s.boards.map((b) =>
          b.id === boardId
            ? { ...b, running: false, serialOutput: "", serialBaudRate: 0 }
            : b,
        );
        const isActive = s.activeBoardId === boardId;
        return {
          boards,
          ...(isActive
            ? { running: false, serialOutput: "", serialBaudRate: 0 }
            : {}),
        };
      });
    },
    ...
    loadBinary: (base64: string) => {
      ...
      if (sim && sim instanceof RP2040Simulator) {
        ...
        console.warn(
          "loadBinary: simulator not initialized or wrong board type",
        );
      }
    },
    ...
    connectRemoteSimulator: (clientId: string) => {
      ...
      if (!bridge) {
        ...
        bridge.onPinChange = (gpioPin, state) => {
          const { wires } = get();
          const sim = getBoardSimulator(
            get().activeBoardId ?? INITIAL_BOARD_ID,
          );
          if (!sim) return;
          const wire = wires.find(
            (w) =>
              ...
              (w.end.componentId.includes("raspberry-pi") &&
                w.end.pinName === String(gpioPin)),
          );
          if (wire) {
            const isArduinoStart =
              !wire.start.componentId.includes("raspberry-pi");
    ...
    updateComponentState: (id, state) => {
      set((prevState) => ({
        components: prevState.components.map((c) =>
          c.id === id
            ? { ...c, properties: { ...c.properties, state, value: state } }
            : c,
        ),
      }));
    },
    ...
    updateWireInProgress: (x, y) =>
      set((state) => {
        ...
        return {
          wireInProgress: { ...state.wireInProgress, currentX: x, currentY: y },
        };
      }),
    ...
    finishWireCreation: (endpoint) => {
      ...
      const finalColor =
        color === DEFAULT_WIRE_COLOR ? autoWireColor(endpoint.pinName) : color;
    ...
    recalculateAllWirePositions: () => {
      ...
      const updatedWires = state.wires.map((wire) => {
        ...
        const startComp = state.components.find(
          (c) => c.id === wire.start.componentId,
        );
        const startBoard = state.boards.find(
          (b) => b.id === wire.start.componentId,
        );
        ...
        const endComp = state.components.find(
          (c) => c.id === wire.end.componentId,
        );
        const endBoard = state.boards.find(
          (b) => b.id === wire.end.componentId,
        );
        const endX = endComp
          ? endComp.x + 6
          : endBoard
      ...
      get().pushCommand({
        ...
        undo: () =>
          set((s) => ({
            ...
            selectedWireId:
              s.selectedWireId === wire.id ? null : s.selectedWireId,
          })),
      });
    },
      ...
      get().pushCommand({
        ...
        execute: () =>
          set((s) => ({
            ...
            selectedWireId:
              s.selectedWireId === wireId ? null : s.selectedWireId,
          })),
...
let lastBoardsRef: readonly BoardInstance[] =
